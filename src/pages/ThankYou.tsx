      ...
      <section className="min-h-screen bg-[#050A14] pt-24 pb-16 relative overflow-hidden flex flex-col justify-center">
        ...
        <div className="container-width relative z-10 max-w-5xl mx-auto px-4 w-full">
          ...
          <div className="text-center mb-12">
            <div
              className={`w-20 h-20 rounded-lg flex items-center justify-center mx-auto mb-6 transition-all duration-500 border backdrop-blur-md ${booked ? "bg-teal-500/10 border-teal-400 shadow-[0_0_30px_rgba(0,229,255,0.2)]" : "bg-[#0A0F1A]/80 border-teal-500/30"}`}
            >
              {booked ? (
                // Glowing neon checkmark
                <svg
                  className="w-10 h-10 text-teal-400 drop-shadow-[0_0_10px_rgba(45,212,191,0.8)]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
            ...
            <p className="font-mono text-sm text-slate-400 max-w-md mx-auto uppercase tracking-wider">
              {booked
                ? "Secure channel initialized. Stand by for discovery procedures."
                : "Awaiting user input to configure discovery schedule."}
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-0 mb-12 font-mono">
            {steps.map((step, i) => (
              <div key={step.label} className="flex items-center">
                ...
                {i < steps.length - 1 && (
                  <div
                    className={`w-8 sm:w-16 h-px mx-2 transition-colors duration-500 ${
                      i === 0 || (i === 1 && booked)
                        ? "bg-teal-400/50"
                        : "bg-white/10"
                    }`}
