 ...
 */
export function FlowingMesh({ opacity = 0.5, parallax = false }: { opacity?: number, parallax?: boolean }) {
    ...
    const draw = () => {
      ...
      for (let j = 0; j < numWaves; j++) {
        ...
        for (let i = 0; i <= width; i += 20) {
          ...
          const dy = Math.sin(x * 0.003 + time + j) * 80 + 
          ...
          if (i === 0) {
            ...
            const prevX = x - 20;
            const prevDy = Math.sin(prevX * 0.003 + time + j) * 80 + 
