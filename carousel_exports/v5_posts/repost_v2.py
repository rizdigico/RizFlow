#!/usr/bin/env python3
"""Delete old posts and repost improved v2 infographics to Instagram."""
import asyncio
import os
import sys
import json
...
async def delete_recent_posts(page, count=10):
    """Delete the most recent `count` posts from Instagram."""
    print(f"\n🗑️ Deleting {count} most recent posts...", flush=True)
    ...
    for attempt in range(count):
        try:
            ...
            if more_btn:
                ...
                print(f"  ⚠️ Could not find more options button for post {attempt+1}", flush=True)
            ...
            delete_btn = None
            menu_items = await page.query_selector_all('[role="dialog"] button, [role="menuitem"]')
            for item in menu_items:
                text = await item.text_content()
                if text and 'delete' in text.strip().lower():
                    delete_btn = item
                    break
            ...
            if delete_btn:
                ...
                if confirm_btn:
                    ...
                    print(f"  ⚠️ Could not confirm delete", flush=True)
                    await page.keyboard.press("Escape")
                    await asyncio.sleep(2)
            else:
                print(f"  ⚠️ Could not find Delete button", flush=True)
                await page.keyboard.press("Escape")
                await asyncio.sleep(2)

            # Close any dialog and go back to profile
            await page.keyboard.press("Escape")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        deleted = await delete_recent_posts(page, count=10)
        print(f"\n{'='*50}", flush=True)
        print(f"Deleted {deleted} old posts", flush=True)
        print(f"{'='*50}\n", flush=True)

        # Step 2: Post new v2 infographics
        results = []
